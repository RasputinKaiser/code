 ...
 * Sibling of ncode_ccr_bridge_multi_environment (multiple envs per host:dir) —
...
async function isMultiSessionSpawnEnabled(): Promise<boolean> {
  return checkGate_CACHED_OR_BLOCKING('ncode_ccr_bridge_multi_session')
}
...
/**
 * Returns the args that must precede CLI flags when spawning a child CLI
 * process. In compiled binaries, process.execPath is the CLI binary itself
...
export async function runBridgeLoop(
  ...
  // the ncode_bridge_repl_v2_cse_shim_enabled gate flips mid-session.
    ...
    for (const [sessionId] of activeSessions) {
      ...
      try {
        ...
        if (err instanceof BridgeFatalError) {
          logEvent('ncode_bridge_heartbeat_error', {
            status:
  ...
  const tokenRefresh = runtimeAuth
    ? createTokenRefreshScheduler({
        getAccessToken: () => runtimeAuth.refreshAccessToken(),
        onRefresh: async sessionId => {
          ...
          if (v2Sessions.has(sessionId)) {
            ...
            const oauthToken = await runtimeAuth.getAccessToken()
            if (oauthToken) {
  ...
  if (isInternalBuild()) {
    ...
    if (config.debugFile) {
      ...
      debugGlob = join(tmpdir(), 'ncode', 'bridge-session-*.log')
    }
  ...
  function onSessionDone(
    ...
    return (rawStatus: SessionDoneStatus): void => {
      ...
      logEvent('ncode_bridge_session_done', {
        status:
  ...
  while (!loopSignal.aborted) {
    ...
    try {
      ...
      if (wasDisconnected) {
        ...
        logEvent('ncode_bridge_reconnected', {
      ...
      if (!work) {
        ...
        if (atCap) {
          ...
          if (pollConfig.non_exclusive_heartbeat_interval_ms > 0) {
            logEvent('ncode_bridge_heartbeat_mode_entered', {
                ...
                : loopSignal.aborted
                  ...
                  : activeSessions.size < config.maxSessions
                    ...
                    : pollDeadline !== null && Date.now() >= pollDeadline
                      ? 'poll_due'
                      : 'config_disabled'
            logEvent('ncode_bridge_heartbeat_mode_exited', {
              reason:
      ...
      try {
        ...
        logEvent('ncode_bridge_work_secret_failed', {})
        ...
        case 'session': {
          ...
          const spawnResult = safeSpawn(
            spawner,
            {
              ...
              sessionIngressLease: buildSessionIngressLease({
          ...
          logEvent('ncode_bridge_session_started', {
            active_sessions: activeSessions.size,
            spawn_mode:
      ...
      if (err instanceof BridgeFatalError) {
        ...
        logEvent('ncode_bridge_fatal_error', {
          status: err.status,
          error_type:
      ...
      if (isConnectionError(err) || isServerError(err)) {
        ...
        if (elapsed >= backoffConfig.connGiveUpMs) {
          ...
          logEvent('ncode_bridge_poll_give_up', {
            error_type:
  ...
  logEvent('ncode_bridge_shutdown', {
    active_sessions: activeSessions.size,
  ...
  // environment alive so `ncode remote-control --session-id=<id>` can resume.
  ...
  if (
    ...
    logger.logStatus(
      `Resume this session by running \`ncode remote-control --continue\``,
    )
    logForDebugging(
...
function onSessionTimeout(
  ...
  logEvent('ncode_bridge_session_timeout', {
...
export function parseArgs(args: string[]): ParsedArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--help' || arg === '-h') {
      ...
      return makeError(
        `Unknown argument: ${arg}\nRun 'ncode remote-control --help' for usage.`,
      )
    }
  }
...
`
    : ''
  const help = `
Remote Control - Connect your local environment to Noumena Code Web

USAGE
  code remote-control [options]
OPTIONS
  --name <name>                    Name for the session (shown in Noumena Code Web)
${
  feature('KAIROS')
...
DESCRIPTION
  ...
  Noumena Code Web (https://console.noumena.com/code). Run this command in the
  directory you want to work in, then connect from the Noumena web app.
${serverDescription}
NOTES
...
export async function bridgeMain(args: string[]): Promise<void> {
  ...
  if (usedMultiSessionFeature && !multiSessionEnabled) {
    await logEventAsync('ncode_bridge_multi_session_denied', {
  ...
  // Resolve auth
  const { getBridgeBaseUrl, getBridgeTokenOverride } = await import(
    './bridgeConfig.js'
  )
  const { resolveManagedRemoteRuntimeAuth } = await import(
    '../auth/capabilities/remote.js'
  )

  let bridgeRuntimeAuth: Awaited<
    ReturnType<typeof resolveManagedRemoteRuntimeAuth>
  >
  try {
    bridgeRuntimeAuth = await resolveManagedRemoteRuntimeAuth({
  ...
  // In production, baseUrl comes from the first-party OAuth config.
  // CLAUDE_BRIDGE_BASE_URL remains a legacy local-dev compatibility override.
  ...
  if (
    ...
    logEvent('ncode_bridge_spawn_mode_chosen', {
      spawn_mode:
  ...
  const api = createBridgeApiClient({
    baseUrl,
    getAccessToken: bridgeRuntimeAuth.getAccessToken,
    runnerVersion: MACRO.VERSION,
    onDebug: logForDebugging,
    onAuth401: bridgeRuntimeAuth.onAuth401,
  ...
  if (feature('KAIROS') && resumeSessionId) {
    ...
    // Proactively refresh the managed bridge auth — getBridgeSession uses raw
    // axios without the withOAuthRetry 401-refresh logic. An expired-but-
    // present token would otherwise produce a misleading "not found" error.
    await bridgeRuntimeAuth.refreshAccessToken()
    const { getBridgeSession } = await import('./createSession.js')
  ...
  try {
    ...
    logEvent('ncode_bridge_registration_failed', {
  ...
  const onStdinData = (data: Buffer): void => {
    ...
    if (data[0] === 0x77 /* 'w' */) {
      ...
      logEvent('ncode_bridge_spawn_mode_toggled', {
        spawn_mode:
...
export async function runBridgeHeadless(
  ...
  if (!checkHasTrustDialogAccepted()) {
    throw new BridgeHeadlessPermanentError(
      `Workspace not trusted: ${dir}. Run the CLI once in that directory first to accept the trust dialog.`,
    )
  }
  ...
  const bridgeRuntimeAuth = await resolveManagedRemoteRuntimeAuthFromCallbacks({
