 ...
 * session ID after resolving fresh auth through `getAccessToken`. The caller
 * is responsible for pulling the current auth value from its runtime seam and
 * delivering it to the appropriate transport (child process stdin for the
 * standalone bridge, WebSocket reconnect for the REPL bridge).
 */
export function createTokenRefreshScheduler({
  ...
  async function doRefresh(sessionId: string, gen: number): Promise<void> {
    ...
    logEvent('ncode_bridge_token_refreshed', {})
