...
type State = {
  ...
  // Session-only flag gating the .ncode/scheduled_tasks.json watcher
  ...
  // .ncode/scheduled_tasks.json — they die with the process. Typed via
  ...
  // Common plain-text headless startup path defers plugin-owned surfaces
  // until after the first query or explicit plugin reconciliation.
  deferHeadlessPluginStartupSurfaces: boolean
  ...
  // Used to compute timeSinceLastApiCallMs in ncode_api_success for
...
function getInitialState(): State {
  ...
  const state: State = {
    ...
    // Remote mode
    isRemoteMode: false,
    deferHeadlessPluginStartupSurfaces: false,
    ...(isInternalBuild()
      ? {
...
export function isDeferredHeadlessPluginStartupSurfacesEnabled(): boolean {
  return STATE.deferHeadlessPluginStartupSurfaces
}

export function setDeferredHeadlessPluginStartupSurfaces(
  enabled: boolean,
): void {
  STATE.deferHeadlessPluginStartupSurfaces = enabled
}
...
export function setMeter(
  ...
  STATE.sessionCounter = createCounter('ncode.session.count', {
    description: 'Count of CLI sessions started',
  })
  STATE.locCounter = createCounter('ncode.lines_of_code.count', {
    description:
      "Count of lines of code modified, with the 'type' attribute indicating whether lines were added or removed",
  })
  STATE.prCounter = createCounter('ncode.pull_request.count', {
    description: 'Number of pull requests created',
  })
  STATE.commitCounter = createCounter('ncode.commit.count', {
    description: 'Number of git commits created',
  })
  STATE.costCounter = createCounter('ncode.cost.usage', {
    description: 'Cost of the NCode session',
    unit: 'USD',
  })
  STATE.tokenCounter = createCounter('ncode.token.usage', {
    description: 'Number of tokens used',
    unit: 'tokens',
  })
  STATE.codeEditToolDecisionCounter = createCounter(
    'ncode.code_edit_tool.decision',
    {
      description:
        'Count of code editing tool permission decisions (accept/reject) for Edit, Write, and NotebookEdit tools',
    },
  )
  STATE.activeTimeCounter = createCounter('ncode.active_time.total', {
