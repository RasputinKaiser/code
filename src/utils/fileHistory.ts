...
export async function fileHistoryTrackEdit(
  ...
  if (!mostRecent) {
    ...
    logEvent('ncode_file_history_track_edit_failed', {})
    return
  }
  ...
  updateFileHistoryState((state: FileHistoryState) => {
    try {
      ...
      logEvent('ncode_file_history_track_edit_success', {
...
export async function fileHistoryMakeSnapshot(
      ...
      Array.from(captured.trackedFiles, async trackingPath => {
        try {
          ...
          if (!fileStats) {
            ...
            logEvent('ncode_file_history_backup_deleted_file', {
              version: nextVersion,
            })
            logForDebugging(
          ...
          logError(error)
          logEvent('ncode_file_history_backup_file_failed', {})
        }
      }),
    )
  }
  ...
  updateFileHistoryState((state: FileHistoryState) => {
    try {
      ...
      logEvent('ncode_file_history_snapshot_success', {
      ...
      logError(error)
      logEvent('ncode_file_history_snapshot_failed', {})
      return state
    }
  })
}
...
export async function fileHistoryRewind(
  ...
  if (!targetSnapshot) {
    ...
    logEvent('ncode_file_history_rewind_failed', {
  ...
  try {
    ...
    logEvent('ncode_file_history_rewind_success', {
      trackedFilesCount: captured.trackedFiles.size,
    ...
    Array.from(state.trackedFiles, async trackingPath => {
      try {
        ...
        if (backupFileName === undefined) {
          ...
          logEvent('ncode_file_history_rewind_restore_file_failed', {
            dryRun: true,
          })
          return null
        }
...
async function createBackup(
  ...
  logEvent('ncode_file_history_backup_file_created', {
    version: version,
...
export async function copyFileHistoryForResume(log: LogOption): Promise<void> {
  ...
  try {
    ...
    if (failedSnapshots > 0) {
      logEvent('ncode_file_history_resume_copy_failed', {
