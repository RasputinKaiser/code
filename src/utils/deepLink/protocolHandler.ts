 ...
 * Protocol Handler
 *
 * Entry point for `ncode --handle-uri <url>`. When the OS invokes claude
...
export async function handleDeepLinkUri(uri: string): Promise<number> {
  ...
  // baked at registration time, and we want the terminal-launched NCode to
...
/**
 * Resolve the working directory for the launched NCode instance.
 * Precedence: explicit cwd > repo lookup (MRU clone) > home.
 * A repo that isn't cloned locally is not an error — fall through to home
 * so a web link referencing a repo the user doesn't have still opens NCode.
 *
