...
export const getCoreUserData = memoize(
  (includeAnalyticsMetadata?: boolean): CoreUserData => {
    ...
    const analyticsSession = getCurrentAnalyticsSession()

    let subscriptionType: string | undefined
    let rateLimitTier: string | undefined
    let firstTokenTime: number | undefined
    if (includeAnalyticsMetadata) {
      subscriptionType = analyticsSession?.subscription.subscriptionType ?? undefined
      rateLimitTier = analyticsSession?.subscription.rateLimitTier ?? undefined
    ...
    const organizationUuid = analyticsSession?.identity.organizationUuid
    const accountUuid = analyticsSession?.identity.accountUuid
...
function getEmail(): string | undefined {
  ...
  const analyticsSession = getCurrentAnalyticsSession()
  if (analyticsSession?.identity.email) {
    return analyticsSession.identity.email
  }
