...
import { getInferenceClient } from '../../services/api/inferenceClient.js'
...
import { getAPIProvider, isFirstPartyNoumenaBaseUrl } from './providers.js'
...
export async function refreshModelCapabilities(): Promise<void> {
  ...
  if (getAuthHeaders().error) {
    logForDebugging('[modelCapabilities] skipped: no auth available')
    return
  }

  try {
    const inferenceClient = await getInferenceClient({ maxRetries: 1 })
    const betas = getCurrentSubscriptionSessionState()
      .isOauthBackedFirstPartySession
      ? [OAUTH_BETA_HEADER]
      : undefined
    const parsed: ModelCapability[] = []
    for await (const entry of inferenceClient.listModels({ betas })) {
