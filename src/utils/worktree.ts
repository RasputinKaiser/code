 ...
 *
 * The slug is joined into the canonical worktree root
 * (`.ncode/worktrees/<slug>` by default, legacy `.claude/worktrees/<slug>`
 * when that storage already exists) via path.join, which
...
export function getWorktreesDir(repoRoot: string): string {
  const canonicalDir = join(repoRoot, '.ncode', 'worktrees')
  const legacyDir = join(repoRoot, '.claude', 'worktrees')
...
//   - directory: `.ncode/worktrees/user/feature/` (or legacy
//     `.claude/worktrees/user/feature/`) lives inside the `user`
...
function worktreePathFor(repoRoot: string, slug: string): string {
  return join(getWorktreesDir(repoRoot), flattenSlug(slug))
}
...
async function getOrCreateWorktree(
  ...
  await mkdir(getWorktreesDir(repoRoot), { recursive: true })
...
export async function createAgentWorktree(slug: string): Promise<{
  ...
  // the main repo's canonical worktree root (.ncode/worktrees by default,
  // legacy .claude/worktrees if it already exists) even when spawned from
  // inside a session worktree — otherwise they nest at
  // <worktree>/.ncode/worktrees and the periodic cleanup (which scans the
 ...
 * Creates the worktree and execs into tmux running NCode inside.
...
export async function execIntoTmuxWorktree(args: string[]): Promise<{
  ...
  // Check if tmux prefix conflicts with NCode keybindings
  // NCode binds: ctrl+b (task:background), ctrl+c, ctrl+d, ctrl+t, ctrl+o, ctrl+r, ctrl+s, ctrl+g, ctrl+e
  const ncodeBindings = [
    'C-b',
    'C-c',
    'C-d',
  ...
  const prefixConflicts = ncodeBindings.includes(tmuxPrefix)

  // Set env vars for the inner NCode to display tmux info in welcome message
  const tmuxEnv = {
    ...process.env,
  ...
  if (shouldSetupDevPanes) {
    // Create detached session with NCode in first pane
    spawnSync(
      'tmux',
      [
        'new-session',
