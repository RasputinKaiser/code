...
import { prefetchCurrentApiKeyHelperIfSafe } from './utils/apiKeyHelperSession.js'
...
export async function setup(
  ...
  if (worktreeEnabled) {
    ...
    logEvent('ncode_worktree_created', { tmux_enabled: tmuxEnabled })
    ...
    // and again at captureHooksConfigSnapshot() above, both from the original
    // dir's project settings (.ncode/settings.json; legacy .claude/settings.json).
    // Re-read from the worktree and re-capture hooks.
    updateHooksConfigSnapshot()
  }
  ...
  // gate, ncode_started beacon, and apiKeyHelper prefetch below must still run.
  if (!isBareMode()) {
    if (isInternalBuild()) {
  ...
  logEvent('ncode_started', {})

  void prefetchCurrentApiKeyHelperIfSafe(getIsNonInteractiveSession()) // Prefetch safely - only executes if trust already confirmed
  ...
  if (
    ...
    if (
      ...
      // Same for CCD (NCode in Desktop) — apps#29127 passes the flag
  ...
  // Log ncode_exit event from the last session?
