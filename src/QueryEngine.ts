...
} from './tools/WebFetchTool/sourceProvenance.js'
import type {
  Message,
  RequestStartEvent,
  StreamEvent,
...
export class QueryEngine {
  ...
  // ncode_skill_tool_invocation). Must persist across the two
  ...
  async *submitMessage(
    ...
    // a memory directory and needs NCode to know how to use it (which
    ...
    const [skills, enabledPlugins] =
      isDeferredHeadlessPluginStartupSurfacesEnabled()
        ? (logForDebugging(
            '[STARTUP] Skipping eager plugin-backed skills/plugins during deferred headless startup',
          ),
          [[], []] as const)
        : await Promise.all([
    ...
    for await (const rawMessage of query({
      ...
      if (message.type === 'assistant') {
        message = repairAssistantMessageWebFetchProvenance(
          message,
          this.mutableMessages,
        )
      }
    ...
    if (result.type === 'assistant') {
      ...
      if (
        ...
        textResult = repairWebFetchProvenanceInText(
          lastContent.text,
          collectWebFetchSourcesFromMessages(this.mutableMessages),
        )
      }
  ...
  /**
   * Prepared-turn execution seam for REPL/local callers.
   *
   * This path accepts fully prepared QueryParams and intentionally bypasses
   * prompt preprocessing (processUserInput, slash command parsing, etc.).
   * It keeps the SDK submitMessage flow unchanged while giving REPL callers
   * a QueryEngine-owned entrypoint for prepared turns.
   */
  static async *runPreparedTurn(
    params: QueryParams,
    deps?: { runQuery?: ReplQueryRunner },
  ): AsyncGenerator<QueryEngineReplEvent, Terminal> {
    const runQuery = deps?.runQuery ?? query
    return yield* runQuery(params)
  }
}

export type QueryEngineReplEvent =
  | StreamEvent
  | RequestStartEvent
  | Message
  | TombstoneMessage
...
type ReplQueryRunner = (
  params: QueryParams,
) => AsyncGenerator<QueryEngineReplEvent, Terminal>

const TOOL_OBLIGATION_PATTERNS = [
  /\b(use|call|run|invoke)\s+(the\s+)?(available\s+)?tools?\b/i,
  /\b(inspect|examine|review|audit|analy[sz]e)\b[\s\S]{0,96}\b(repo|repository|codebase|project|files?|source|diff)\b/i,
  /\b(repo|repository|codebase|project|files?|source|diff)\b[\s\S]{0,96}\b(inspect|examine|review|audit|analy[sz]e)\b/i,
  /\b(read|grep|search|list|find)\b[\s\S]{0,96}\b(files?|repo|repository|codebase|project)\b/i,
]
...
function hasActionableTools(tools: Tools): boolean {
  return tools.some(tool =>
    [
      ...
      'Write',
      'NotebookRead',
    ].some(name => toolMatchesName(tool, name)),
  )
}

export function inferInitialToolChoiceFromPrompt(
  prompt: string | Array<ContentBlockParam>,
  tools: Tools,
  explicitToolChoice?: BetaToolChoiceTool | BetaToolChoiceAuto,
): BetaToolChoiceTool | BetaToolChoiceAuto | undefined {
  if (explicitToolChoice || !hasActionableTools(tools)) {
    return explicitToolChoice
  }

  const text = promptTextForToolObligation(prompt)
  if (!text.trim()) {
    return undefined
  }

  return TOOL_OBLIGATION_PATTERNS.some(pattern => pattern.test(text))
    ? { type: 'any' }
    : undefined
}

/**
 * REPL-facing QueryEngine seam.
 *
 * The interactive REPL still consumes the legacy local event union while the
 * broader engine converges on QueryEngine-owned session logic. This wrapper
 * gives the REPL a QueryEngine entrypoint without changing its render/update
...
export async function* runReplTurn(
  ...
  return yield* QueryEngine.runPreparedTurn(params, deps)
}
...
export async function* ask({
  ...
  const initialToolChoice = inferInitialToolChoiceFromPrompt(
    prompt,
    tools,
    toolChoice,
  )
