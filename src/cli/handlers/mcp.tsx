 ...
 * These are dynamically imported only when the corresponding `ncode mcp *` command runs.
 */
...
export async function mcpAddFromDesktopHandler(options: {
  ...
  try {
    ...
    if (Object.keys(servers).length === 0) {
      cliOk('No MCP servers found in NCode Desktop configuration or configuration file does not exist.');
    }
    const {
      unmount
...
export async function mcpResetChoicesHandler(): Promise<void> {
  logEvent('ncode_mcp_reset_mcpjson_choices', {});
  ...
  cliOk('All project-scoped (.mcp.json) server approvals and rejections have been reset.\n' + 'You will be prompted for approval next time you start Code.');
}
