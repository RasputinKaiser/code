...
export class StructuredIO {
   ...
   * Used by the bridge to feed permission responses from the remote client into the
   * SDK permission flow.
   *
   ...
   * stale permission prompt on the remote client when the SDK consumer wins the race.
   */
  ...
  private async processLine(
    ...
    try {
      ...
      if (message.type === 'control_response') {
        ...
        // request, so it can cancel the stale permission prompt on the remote client.
        if (
      ...
      if (message.type === 'control_request') {
        if (!message.request) {
          cliError(`Error: Missing request on control_request`)
        }
        return message
      }
