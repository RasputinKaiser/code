...
import {
  EXTERNAL_GROWTHBOOK_CLIENT_KEY,
...
import { resolveGrowthBookApiHost } from './growthbookHost.js'
...
// Cache evaluated feature values for the current user/session. In premium
// remote-eval mode we hydrate this directly from the server payload; in the
// standard OSS/self-hosted path we compute it locally from the fetched feature
// definitions after setPayload().
const resolvedFeatureValues = new Map<string, unknown>()
...
// ncode_1p_event_batch_config once and builds a LoggerProvider with it) and
 ...
 * (resolvedFeatureValues is populated), the listener fires once on the
...
export function onGrowthBookRefresh(
  ...
  if (resolvedFeatureValues.size > 0) {
    queueMicrotask(() => {
      ...
      if (subscribed && resolvedFeatureValues.size > 0) {
        callSafe(listener)
      }
    })
  }
...
export function hasGrowthBookEnvOverride(feature: string): boolean {
  ...
  return (
    overrides !== null &&
    getLegacyCompatibleValue(overrides, feature) !== undefined
  )
}
...
export function getAllGrowthBookFeatures(): Record<string, unknown> {
  if (resolvedFeatureValues.size > 0) {
    return Object.fromEntries(resolvedFeatureValues)
  }
 ...
 * otherwise overriding e.g. ncode_internal_model_override wouldn't actually
...
export function setGrowthBookConfigOverride(
  ...
  try {
    saveGlobalConfig(c => {
      ...
      if (value === undefined) {
        if (!(canonicalFeature in current)) return c
        const { [canonicalFeature]: _, ...rest } = current
      ...
      if (isEqual(current[canonicalFeature], value)) return c
      return { ...c, growthBookOverrides: { ...current, [canonicalFeature]: value } }
    })
...
function logExposureForFeature(feature: string): void {
  ...
  if (loggedExposures.has(canonicalFeature)) {
    return
  }

  const expData =
    experimentDataByFeature.get(canonicalFeature) ??
    experimentDataByFeature.get(feature)
  if (expData) {
    loggedExposures.add(canonicalFeature)
    logGrowthBookExperimentTo1P({
      experimentId: expData.experimentId,
      variationId: expData.variationId,
      userAttributes: getUserAttributes(),
      experimentMetadata: {
        feature_id: canonicalFeature,
      },
    })
  }
}
...
function updateExperimentDataForFeature(
  ...
  const canonicalFeature = toCanonicalAnalyticsName(feature)
  if (
    evaluation.source !== 'experiment' ||
    !evaluation.experiment?.key ||
    evaluation.experimentResult?.variationId === undefined
  ) {
    experimentDataByFeature.delete(canonicalFeature)
    return
  }

  experimentDataByFeature.set(canonicalFeature, {
    experimentId: evaluation.experiment.key,
    variationId: evaluation.experimentResult.variationId,
    inExperiment: evaluation.experimentResult.inExperiment,
    hashAttribute: evaluation.experimentResult.hashAttribute,
    hashValue: evaluation.experimentResult.hashValue,
  })
}

/**
 * Process a GrowthBook payload from the server and populate local caches.
 * Called after both initial client.init() and after
 ...
 *
 * Without this running on refresh, resolvedFeatureValues freezes at its
 * init-time snapshot and getDynamicConfig_BLOCKS_ON_INIT returns stale values
 * for the entire process lifetime — which broke the ncode_max_version_config
 * kill switch for long-running sessions.
 */
async function processGrowthBookPayload(
  gbClient: GrowthBook,
): Promise<boolean> {
  // Premium remote-eval payloads use { value: ... } while the standard
  // feature-definition endpoint uses { defaultValue, rules }. Normalize the
  // pre-evaluated shape so the SDK can consume either response.
  ...
  // below, return true, and syncGrowthBookCacheToDisk would wholesale-write `{}`
  // to disk: total flag blackout for every process sharing ~/.ncode.json.
  ...
  const hasPreEvaluatedValues = Object.values(transformedFeatures).some(
    feature =>
      'value' in feature || 'source' in feature || 'experimentResult' in feature,
  )

  resolvedFeatureValues.clear()
  for (const [key, feature] of Object.entries(transformedFeatures)) {
    if (hasPreEvaluatedValues) {
      // Under premium remote-eval the server has already decided the value for
      // this user. Preserve that answer directly instead of re-evaluating
      // locally inside the SDK.
      const evaluatedValue =
        'value' in feature ? feature.value : feature.defaultValue
      if (evaluatedValue !== undefined) {
        resolvedFeatureValues.set(key, evaluatedValue)
      }
      updateExperimentDataForFeature(key, feature as FeatureEvaluationSnapshot)
      continue
    }

    // Standard self-hosted path: fetch feature definitions and evaluate them
    // locally for the current user/session so cache + refresh semantics stay
    // identical to the remote-eval lane.
    const result = gbClient.evalFeature(key)
    if (result.value !== undefined && result.value !== null) {
      resolvedFeatureValues.set(key, result.value)
    } else if (feature.defaultValue !== undefined) {
      resolvedFeatureValues.set(key, feature.defaultValue)
    }
    updateExperimentDataForFeature(key, result)
  }
  return true
}

/**
 * Write the complete resolvedFeatureValues map to disk. Called exactly
 * once per successful processGrowthBookPayload — never from a failure path,
 ...
 * process's SDK key.
 */
function syncGrowthBookCacheToDisk(): void {
  const fresh = Object.fromEntries(resolvedFeatureValues)
...
function getGrowthBookInitErrorMessage(error: unknown): string {
...
function isInvalidGrowthBookClientKey(error: unknown): boolean {
  return getGrowthBookInitErrorMessage(error).includes('Invalid SDK key')
}

function getGrowthBookApiHost(): string {
  const legacyAnthropicOverride =
    isInternalBuild()
      ? process.env.CLAUDE_CODE_GB_BASE_URL?.trim()
      : undefined

  return resolveGrowthBookApiHost({
    noumenaOverride: process.env.NOUMENA_GROWTHBOOK_API_HOST,
    legacyAnthropicOverride,
    platformBaseUrl: process.env.NOUMENA_PLATFORM_BASE_URL || undefined,
  })
}
...
const getGrowthBookClient = memoize(
  (): { client: GrowthBook; initialized: Promise<void> } | null => {
    ...
    const baseUrl = getGrowthBookApiHost()

    if (!clientKey || !baseUrl) {
      // Public builds without explicit config fall back to defaults and disk
      // cache instead of contacting an unconfigured feature service.
      logForDebugging(
        'GrowthBook: client key or API host not configured, skipping remote feature service',
      )
      return null
    }
    ...
    const createClient = (selectedClientKey: string): GrowthBook =>
      new GrowthBook({
        apiHost: baseUrl,
        ...
        onFeatureUsage: (feature, result) => {
          updateExperimentDataForFeature(feature, result)
        },
    ...
    const finalizeInit = async (
      ...
      if (isInternalBuild()) {
        logForDebugging(
          `GrowthBook initialized successfully, source: ${result.source}, success: ${result.success}, clientKey: ${selectedClientKey}`,
        )
        if (!result.success) {
          logForDebugging(
            `GrowthBook init failure detail: ${getGrowthBookInitErrorMessage(result.error)}`,
          )
        }
      }
      ...
      if (
        isInternalBuild() &&
        !result.success &&
        selectedClientKey !== EXTERNAL_GROWTHBOOK_CLIENT_KEY &&
        isInvalidGrowthBookClientKey(result.error)
      ) {
        logForDebugging(
          `GrowthBook: Client key ${selectedClientKey} rejected by project registry, retrying with external project key`,
        )
        growthBookClient.destroy()
        const fallbackClient = createClient(EXTERNAL_GROWTHBOOK_CLIENT_KEY)
        client = fallbackClient
        const fallbackResult = await fallbackClient.init({ timeout: 5000 })
        await finalizeInit(
          fallbackClient,
          fallbackResult,
          EXTERNAL_GROWTHBOOK_CLIENT_KEY,
      ...
      const hadFeatures = await processGrowthBookPayload(growthBookClient)
      // Re-check: processGrowthBookPayload yields at `await setPayload`.
      ...
      // this runs after.
      if (client !== growthBookClient) return
      ...
      if (hadFeatures) {
        ...
        syncGrowthBookCacheToDisk()
        // Notify subscribers: resolvedFeatureValues is populated and
      ...
      if (isInternalBuild()) {
        const features = growthBookClient.getFeatures()
        if (features) {
    ...
    const thisClient = createClient(clientKey)
    client = thisClient
    ...
    const initialized = thisClient
      .init({ timeout: 5000 })
      .then(result => finalizeInit(thisClient, result, clientKey))
      .catch(error => {
        if (isInternalBuild()) {
...
async function getFeatureValueInternal<T>(
  ...
  const overrides = getEnvOverrides()
  const override = getLegacyCompatibleValue(overrides ?? undefined, canonicalFeature)
  if (override !== undefined) {
    return override as T
  }
  const configOverrides = getConfigOverrides()
  const configOverride = getLegacyCompatibleValue(configOverrides, canonicalFeature)
  if (configOverride !== undefined) {
    return configOverride as T
  }
  ...
  let result: T
  const resolvedFeatureValue = getLegacyCompatibleValue(
    Object.fromEntries(resolvedFeatureValues) as Record<string, unknown>,
    canonicalFeature,
  )
  if (resolvedFeatureValue !== undefined) {
    result = resolvedFeatureValue as T
  } else {
    result = growthBookClient.getFeatureValue(canonicalFeature, defaultValue) as T
  }

  // Log experiment exposure using stored experiment data
  if (logExposure) {
    logExposureForFeature(canonicalFeature)
  }

  if (isInternalBuild()) {
    logForDebugging(
      `GrowthBook: getFeatureValue("${canonicalFeature}") = ${jsonStringify(result)}`,
    )
  }
  return result
}
 ...
 * populated by syncGrowthBookCacheToDisk on every successful payload (init +
...
export function getFeatureValue_CACHED_MAY_BE_STALE<T>(
  ...
  if (experimentDataByFeature.has(canonicalFeature)) {
    logExposureForFeature(canonicalFeature)
  } else {
    pendingExposures.add(canonicalFeature)
  }

  // In-memory payload is authoritative once processGrowthBookPayload has run.
  // Disk is also fresh by then (syncGrowthBookCacheToDisk runs synchronously inside
  ...
  if (resolvedFeatureValue !== undefined) {
    return resolvedFeatureValue as T
  }

  // Fall back to disk cache (survives across process restarts)
  try {
    const cached = getLegacyCompatibleValue(
...
export function checkStatsigFeatureGate_CACHED_MAY_BE_STALE(
  ...
  const canonicalGate = toCanonicalAnalyticsName(gate)
  // Check env var overrides first (for eval harnesses)
  const overrides = getEnvOverrides()
  const override = getLegacyCompatibleValue(overrides ?? undefined, canonicalGate)
  if (override !== undefined) {
    return Boolean(override)
  }
  const configOverrides = getConfigOverrides()
  const configOverride = getLegacyCompatibleValue(configOverrides, canonicalGate)
  if (configOverride !== undefined) {
    return Boolean(configOverride)
  }
  ...
  // Log experiment exposure if data is available, otherwise defer until after init
  if (experimentDataByFeature.has(canonicalGate)) {
    logExposureForFeature(canonicalGate)
  } else {
    pendingExposures.add(canonicalGate)
  }

  // Return cached value immediately from disk
  // First check GrowthBook cache, then fall back to Statsig cache for migration
  const config = getGlobalConfig()
  const gbCached = getLegacyCompatibleValue(
  ...
  return Boolean(
    getLegacyCompatibleValue(config.cachedStatsigGates, canonicalGate) ?? false,
  )
}
...
export async function checkSecurityRestrictionGate(
  ...
  const config = getGlobalConfig()
  const statsigCached = getLegacyCompatibleValue(
    config.cachedStatsigGates,
    canonicalGate,
  )
 ...
 * fresh server value (max ~5s). Disk is populated by syncGrowthBookCacheToDisk
...
export async function checkGate_CACHED_OR_BLOCKING(
  ...
  return getFeatureValueInternal(canonicalGate, false, true)
}
...
export function refreshGrowthBookAfterAuthChange(): void {
  ...
  try {
    ...
    // resetGrowthBook cleared resolvedFeatureValues. If re-init below
...
export async function refreshGrowthBookFeatures(): Promise<void> {
  ...
  try {
    ...
    // Rebuild the evaluated value cache from the refreshed payload so that
    ...
    // processGrowthBookPayload (the guard above only covers refreshFeatures).
    ...
    // resolvedFeatureValues wasn't rebuilt — skip both the no-op disk
