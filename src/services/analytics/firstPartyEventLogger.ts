...
const EVENT_SAMPLING_CONFIG_NAME = 'ncode_event_sampling_config'
/**
...
const BATCH_CONFIG_NAME = 'ncode_1p_event_batch_config'
type BatchConfig = {
 ...
 *
 * @param eventName - Name of the event (e.g., 'ncode_api_query')
...
async function logEventTo1PAsync(
  ...
  try {
    ...
    if (isInternalBuild()) {
      logForDebugging(
        `[NOUMENA-ONLY] 1P event: ${eventName} ${jsonStringify(metadata, null, 0)}`,
      )
    }
...
export function logGrowthBookExperimentTo1P(
  ...
  if (isInternalBuild()) {
    logForDebugging(
      `[NOUMENA-ONLY] 1P GrowthBook experiment: ${data.experimentId} variation=${data.variationId}`,
    )
  }
