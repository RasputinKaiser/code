...
import { memoizeAsyncWithLRU, memoizeWithLRU } from '../../utils/memoize.js'
...
export function getMcpSessionIngressTransportHeaders(params?: {
  includeWhenServerHasOAuthTokens?: boolean
}): Record<string, string> {
  const sessionIngressHeaders = getSessionIngressAuthHeaders()
  if (Object.keys(sessionIngressHeaders).length === 0) {
    return {}
  }

  if (params?.includeWhenServerHasOAuthTokens === false) {
    return {}
  }

  return sessionIngressHeaders
}
...
// Lazy: toolRendering.tsx pulls React/ink; only needed when NCode-in-Chrome MCP server is connected
/* eslint-disable @typescript-eslint/no-require-imports */
...
// GrowthBook ncode_malort_pedway (see gates.ts).
...
/**
 * Shared handler for sse/http/managed-proxy auth failures during connect:
 * emits ncode_mcp_server_needs_auth, caches the needs-auth entry, and returns
...
function handleRemoteAuthFailure(
  ...
  logEvent('ncode_mcp_server_needs_auth', {
    transportType:
...
/**
 * Fetch wrapper for managed proxy connections. Attaches the OAuth bearer
  ...
  return async (url, init) => {
    const doRequest = async () => {
      ...
      if (
        ...
        throw new Error('No managed Noumena OAuth token available')
      }
      // eslint-disable-next-line eslint-plugin-n/no-unsupported-features/node-builtins
      const headers = new Headers(init?.headers)
      headers.set('Authorization', `Bearer ${session.accessToken}`)
      const response = await innerFetch(url, { ...init, headers })
      return { response, sentToken: session.accessToken }
    }
    ...
    const refreshedSession = await getAuthRuntime()
      .resolveSession({
        allowRefresh: true,
        forceRefresh: true,
      })
      .catch(() => null)
    const nextToken =
      refreshedSession?.principalSource === 'managed_oauth' &&
      refreshedSession.sessionState === 'usable'
        ? refreshedSession.accessToken
        : null
    const tokenChanged = Boolean(nextToken && nextToken !== sentToken)
    logEvent('ncode_mcp_managed_proxy_401', {
      tokenChanged:
 ...
 */
export const connectToServer = memoizeAsyncWithLRU(
  async (
    name: string,
    serverRef: ScopedMcpServerConfig,
    serverStats?: {
    ...
    try {
      ...
      const sessionIngressHeaders = getMcpSessionIngressTransportHeaders()
      ...
      if (serverRef.type === 'sse') {
        ...
        const wsHeaders = {
          'User-Agent': getMCPUserAgent(),
          ...(serverRef.authToken && {
            'X-NCode-Ide-Authorization': serverRef.authToken,
          }),
        }
        ...
        logMCPDebug(
          ...
          `WebSocket transport options: ${jsonStringify({
            ...
            hasSessionAuth: Object.keys(sessionIngressHeaders).length > 0,
          })}`,
        )
          ...
          requestInit: {
            ...
            headers: {
              ...
              ...getMcpSessionIngressTransportHeaders({
                includeWhenServerHasOAuthTokens: !hasOAuthTokens,
              }),
              ...combinedHeaders,
            },
          },
      ...
      } else if (serverRef.type === 'managed-proxy') {
        logMCPDebug(
          name,
          `Initializing managed proxy transport for server ${serverRef.id}`,
        )

        const managedSession = getAuthRuntime().getCurrentManagedSession()
        if (!managedSession || (!managedSession.accessToken && !managedSession.canRefresh)) {
          throw new Error('No managed Noumena OAuth token found')
        }

        const oauthConfig = getOauthConfig()
        const proxyUrl = `${oauthConfig.MCP_PROXY_URL}${oauthConfig.MCP_PROXY_PATH.replace('{server_id}', serverRef.id)}`

        logMCPDebug(name, `Using managed proxy at ${proxyUrl}`)
        ...
        logMCPDebug(name, `managed proxy transport created successfully`)
      } else if (
      ...
      const client = new Client(
        {
          ...
          description: "Noumena's agentic coding tool",
      ...
      try {
        ...
        if (serverRef.type === 'sse' && error instanceof Error) {
          ...
          serverRef.type === 'managed-proxy' &&
          error instanceof Error
        ) {
          logMCPDebug(
            name,
            `managed proxy connection failed after ${elapsed}ms: ${error.message}`,
          )
          logMCPError(name, error)
          ...
          if (errorCode === 401) {
            return handleRemoteAuthFailure(name, serverRef, 'managed-proxy')
          }
        } else if (
          serverRef.type === 'sse-ide' ||
          serverRef.type === 'ws-ide'
        ) {
          logEvent('ncode_mcp_ide_server_connection_failed', {
      ...
      if (serverRef.type === 'sse-ide' || serverRef.type === 'ws-ide') {
        ...
        logEvent('ncode_mcp_ide_server_connection_succeeded', {
      ...
      client.onerror = (error: Error) => {
        ...
        if (
          (transportType === 'http' || transportType === 'managed-proxy') &&
      ...
      logEvent('ncode_mcp_server_connection_succeeded', {
      ...
      const connectionDurationMs = Date.now() - connectStartTime
      logEvent('ncode_mcp_server_connection_failed', {
        connectionDurationMs,
  ...
  async (client: MCPServerConnection): Promise<Tool[]> => {
    ...
    try {
      ...
      const skipPrefix =
        ...
        isEnvTruthy(process.env.NCODE_AGENT_SDK_MCP_NO_PREFIX)
  ...
  return new Promise(resolve => {
    ...
    getMcpToolsCommandsAndResources(result => {
      ...
      if (completedCount >= pendingCount) {
        ...
        logEvent('ncode_mcp_tools_commands_loaded', {
...
export async function processMCPResult(
  ...
  if (isEnvDefinedFalsy(process.env.ENABLE_MCP_LARGE_OUTPUT_FILES)) {
    logEvent('ncode_mcp_large_result_handled', {
      outcome: 'truncated',
      reason: 'env_disabled',
      sizeEstimateTokens,
...
async function callMCPTool({
  ...
  try {
    ...
    if (codeIndexingTool) {
      logEvent('ncode_code_indexing_tool_used', {
    ...
    if (e instanceof Error) {
      ...
      if (errorCode === 401 || e instanceof UnauthorizedError) {
        ...
        logEvent('ncode_mcp_tool_call_auth_error', {})
      ...
      const isConnectionClosedOnHttp =
        ...
        (config.type === 'http' || config.type === 'managed-proxy')
      if (isSessionExpired || isConnectionClosedOnHttp) {
        ...
        logEvent('ncode_mcp_session_expired', {})
        await clearServerCache(name, config)
