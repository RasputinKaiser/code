 ...
 * notify VSCode when files are edited or written by NCode.
 */
...
export function setupVscodeSdkMcp(sdkClients: MCPServerConnection[]): void {
  ...
  if (client && client.type === 'connected') {
    ...
    const gates: Record<string, boolean | string> = {
      ncode_vscode_review_upsell: checkStatsigFeatureGate_CACHED_MAY_BE_STALE(
        'ncode_vscode_review_upsell',
      ),
      ncode_vscode_onboarding: checkStatsigFeatureGate_CACHED_MAY_BE_STALE(
        'ncode_vscode_onboarding',
      ),
      // Browser support.
      ncode_quiet_fern: getFeatureValue_CACHED_MAY_BE_STALE(
        'ncode_quiet_fern',
        false,
      ),
      // In-band OAuth via claude_authenticate (vs. extension-native PKCE).
      ncode_vscode_cc_auth: getFeatureValue_CACHED_MAY_BE_STALE(
        'ncode_vscode_cc_auth',
        false,
      ),
    }
    ...
    if (autoModeState !== undefined) {
      gates.ncode_auto_mode_state = autoModeState
    }
