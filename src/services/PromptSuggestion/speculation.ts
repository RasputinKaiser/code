...
import { getNcodeTempDir } from '../../utils/permissions/filesystem.js'
...
function getOverlayPath(id: string): string {
  return join(getNcodeTempDir(), 'speculation', String(process.pid), id)
}
...
function createSpeculationFeedbackMessage(
  ...
  return createSystemMessage(
    `[NOUMENA-ONLY] ${parts.join(' · ')} · ${savedText}${sessionSuffix}`,
    'warning',
  )
}
